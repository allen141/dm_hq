...
- The 2D camera uses a fixed right-angle polar constraint relative to its screen-orientation axis, pans in screen space, and projects every target back onto the map plane before applying zoom and pan bounds. Zooming, panning, resizing, and resetting cannot turn the camera edge-on, move the entire image outside the viewport, or carry the camera through the plane.
