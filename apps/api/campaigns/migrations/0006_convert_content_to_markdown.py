        ...
        with transaction.atomic():
            ...
            for item in ArchiveItem.objects.filter(campaign_id=campaign.id).all():
                ...
                metadata = {
                    ...
                    "tags": list(item.item_tags.values_list("tag__name", flat=True)),
                    "references": [
                        {"target_id": str(r.target_id), "label": r.label} for r in item.outgoing_references.all()
                    ],
                    "relationships": [
                        {
                            "target_id": str(r.target_id),
                            "kind": r.kind,
                            "reciprocal_label": r.reciprocal_label,
                            "notes": r.notes,
                        }
                ...
                create_version(
                    ...
                    campaign,
                    "archive_item",
                    f"campaigns/{campaign.id}/items/{item.id}.md",
                    value,
                    campaign.owner,
                    relation=item,
                )
            for version in (
                TemplateVersion.objects.filter(template__campaign_id=campaign.id).select_related("template").all()
            ):
                ...
                create_version(
                    ...
                    campaign,
                    "template",
                    f"campaigns/{campaign.id}/templates/{version.template_id}/v{version.number}.md",
                    value,
                    campaign.owner,
                    relation=version,
                )
            for entry in (
                PublicationEntry.objects.filter(version__publication__campaign_id=campaign.id)
                .select_related("version__publication", "item")
                .all()
            ):
                ...
                metadata = {
                    ...
                    "id": str(uuid.uuid4()),
                    "campaign_id": str(campaign.id),
                    "source_item_id": str(entry.item_id),
                    "title": entry.safe_title,
                    "fields": entry.safe_fields or {},
                    "status": "published",
                }
