...
export default function Home() {
  ...
  const [username, setUsername] = useState("dm");
  ...
  async function refresh() {
    try {
      ...
      setUser(current);
      setCampaigns(await client.campaigns());
    } catch {
      setUser(null);
    }
  }
  ...
  async function signIn(event: FormEvent) {
    ...
    try {
      const current = await client.login(username, password);
      setUser(current); setCampaigns(await client.campaigns());
  ...
  async function createCampaign(event: FormEvent) {
    ...
    try {
      ...
      setCampaigns((current) => [campaign, ...current]); setSelectedCampaign(campaign); setCampaignName("");
  ...
  async function openCampaign(campaignId: string) {
    setBusy(true); setError("");
    try {
      setSelectedCampaign(await client.campaign(campaignId));
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Campaign could not be opened.");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function signOut() {
    await client.logout();
    setUser(null); setCampaigns([]); setSelectedCampaign(null);
  }

  return (
    <main className="shell">
      <header className="masthead">
        <div><div className="eyebrow">The Archive · walking scaffold</div><h1>Keep the thread.</h1></div>
        <p className="lede">A quiet home for campaign knowledge. Start with a campaign container today; notes, entities, and relationships will grow from the same foundation.</p>
      </header>
