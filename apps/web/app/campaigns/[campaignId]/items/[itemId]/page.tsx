...
export default function ItemPage() {
  ...
  async function load() {
    try {
      const [current, history] = await Promise.all([client.item(params.itemId), client.revisions(params.itemId)]);
      setItem(current); setRevisions(history.revisions); setTitle(current.title); setBody(current.body); setStatus(current.status);
      setFields(JSON.stringify(current.entity?.fields ?? {}, null, 2)); setAliases(current.aliases.join(", ")); setTags(current.tags.join(", "));
      setScheduledFor(current.session?.scheduled_for ?? ""); setSessionStatus(current.session?.session_status ?? "planned"); setOutcomeText(current.session?.outcome_text ?? "");
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Item could not be loaded."); }
  }
  ...
  async function save(event: FormEvent) {
    event.preventDefault(); if (!item) return; setBusy(true); setError(""); setMessage("");
    try {
      const parsed = item.kind === "entity" ? JSON.parse(fields) as Record<string, unknown> : undefined;
      const updated = await client.updateItem(item.id, { version: item.version, title, body, status, fields: parsed, aliases: aliases.split(","), tags: tags.split(","), scheduled_for: scheduledFor || undefined, session_status: sessionStatus, outcome_text: outcomeText });
      setItem(updated); setMessage("Saved as a new revision."); await load();
  ...
  async function publish() { if (!item) return; try { const publication = await client.createPublication(params.campaignId, [{ item_id: item.id, title: item.title, body: includeBody ? item.body : "", fields: includeFields ? item.entity?.fields ?? {} : {} }]); setShareUrl(`${window.location.origin}${publication.url ?? `/p/${publication.token ?? ""}`}`); setMessage("Player snapshot published."); } catch (cause) { setError(cause instanceof Error ? cause.message : "Publication failed."); } }
  ...
  return <main className="shell"><header className="masthead"><div><Link className="back-link" href={`/campaigns/${params.campaignId}`}>← Campaign</Link><div className="eyebrow">{item.kind} · {item.status}</div><h1>{item.title}</h1></div><p className="lede">This is a durable campaign record. Private source content stays separate from anything you publish.</p></header><div className="workspace archive-workspace"><section className="panel"><form onSubmit={save}><label>Title<input value={title} onChange={(event) => setTitle(event.target.value)} /></label><label>Status<select value={status} onChange={(event) => setStatus(event.target.value)}><option value="draft">Draft</option><option value="canon">Canon</option><option value="archived">Archived</option></select></label><label>Aliases<input value={aliases} onChange={(event) => setAliases(event.target.value)} placeholder="Mara, The Ferrymaster" /></label><label>Tags<input value={tags} onChange={(event) => setTags(event.target.value)} placeholder="harbor, faction" /></label><label>Markdown source<textarea value={body} onChange={(event) => setBody(event.target.value)} rows={12} /></label>{item.kind === "entity" && <label>Template fields (JSON)<textarea value={fields} onChange={(event) => setFields(event.target.value)} rows={8} /></label>}{item.kind === "session" && <><label>Scheduled date<input type="date" value={scheduledFor} onChange={(event) => setScheduledFor(event.target.value)} /></label><label>Session status<select value={sessionStatus} onChange={(event) => setSessionStatus(event.target.value)}><option value="planned">Planned</option><option value="completed">Completed</option></select></label><label>Outcome / scratch capture<textarea value={outcomeText} onChange={(event) => setOutcomeText(event.target.value)} rows={7} /></label></>}<button disabled={busy}>{busy ? "Saving…" : "Save revision"}</button></form><div className="preview"><div className="eyebrow">Sanitized preview</div><div dangerouslySetInnerHTML={{ __html: item.html }} /></div></section><aside className="panel"><div className="eyebrow">Next actions</div>{item.kind === "entity" && <div className="publication-options"><div className="eyebrow">Player-safe selection</div><label className="checkbox-label"><input type="checkbox" checked={includeBody} onChange={(event) => setIncludeBody(event.target.checked)} /> Include prose</label><label className="checkbox-label"><input type="checkbox" checked={includeFields} onChange={(event) => setIncludeFields(event.target.checked)} /> Include selected fields</label></div>}<div className="action-stack">{item.kind === "note" && <button className="secondary" onClick={promote}>Promote to Person / NPC</button>}<button className="secondary" onClick={publish}>Publish player snapshot</button><button className="secondary" onClick={exportCampaign}>Export campaign</button></div>{shareUrl && <p className="success">Player link: <a href={shareUrl}>{shareUrl}</a></p>}<form onSubmit={connect} className="connect-form"><div className="eyebrow">Connect records</div><label>Target item UUID<input value={targetId} onChange={(event) => setTargetId(event.target.value)} placeholder="Paste another item ID" /></label><label>Relationship kind<input value={relationshipKind} onChange={(event) => setRelationshipKind(event.target.value)} /></label><button className="secondary">Add relationship</button>{item.kind === "session" && <button type="button" className="secondary" onClick={() => void linkSession()}>Link target to session</button>}</form>{revisions.length > 1 && <div className="revision-list"><div className="eyebrow">Revision history</div>{revisions.map((revision) => <div className="revision-row" key={revision.number}><span className="meta">v{revision.number} · {revision.reason || "Saved"}</span>{revision.number !== item.version && <button type="button" className="secondary" onClick={() => void restoreRevision(revision.number)}>Restore</button>}</div>)}</div>}<div className="connect-form"><div className="eyebrow">Restore campaign export</div><input type="file" accept=".zip,application/zip" onChange={(event) => setRestoreFile(event.target.files?.[0] ?? null)} /><button type="button" className="secondary" disabled={!restoreFile} onClick={() => void restoreCampaign()}>Restore as new campaign</button></div>{item.relationships.length > 0 && <div><div className="eyebrow">Relationships</div>{item.relationships.map((relationship) => <p className="meta" key={relationship.id}>{relationship.kind} → {relationship.target_id}</p>)}</div>}{message && <p className="success" role="status">{message}</p>}{error && <p className="error" role="alert">{error}</p>}</aside></div></main>;
}
