...
export default function ItemPage() {
  ...
  async function load() {
    try {
      const [current, history, publicationResult] = await Promise.all([client.item(params.itemId), client.revisions(params.itemId), client.publications(params.campaignId)]);
      setItem(current); setRevisions(history.revisions); setHandouts(publicationResult.publications.filter((publication) => publication.item_ids.includes(params.itemId))); setTitle(current.title); setBody(current.body); setStatus(current.status);
